...
(function () {
  ...
  function intercept(e, ctx) {
    ...
    // Язык собеседника ещё не известен — блокируем и определяем по сообщениям тикета
    block(e);
    checking = true;
  ...
  // --- режим 'auto': отправляет ли сайт обычным Enter ---
  // Обычный Enter не блокируем. Если после него поле опустело (сообщение ушло) — запоминаем для этого правила,
  // и дальше обычный Enter тоже проверяется. Если Enter переносит строку — ничего не меняется.
  let learned = {};
  chrome.storage.local.get('sendKeyLearned').then((r) => { learned = (r && r.sendKeyLearned) || {}; }).catch(() => {});
  function enterSends() {
    return !!(ST.site && learned[ST.site.id] === 'enter');
  }
  function learnEnter(e) {
    const t = e.composedPath()[0];
    if (!ST.site || enterSends() || !ST.fieldAllowed(t)) return;
    const field = ST.editableRoot(t);
    const before = ST.getFieldText(field).trim();
    if (!before) return;
    setTimeout(() => {
      const f = field.isConnected ? field : findField();
      if (!f || ST.getFieldText(f).trim() || !ST.site) return;
      learned[ST.site.id] = 'enter';
      chrome.storage.local.set({ sendKeyLearned: learned }).catch(() => {});
      ST.log('Сайт отправляет сообщения по Enter — защита будет проверять и Enter');
    }, 600);
  }
  ...
  window.addEventListener('keydown', (e) => {
    ...
    // 'auto': обычный Enter проверяем, только если заметили, что сайт им отправляет. Остальные Enter (новая строка) не трогаем.
    const explicit =
      (sendKey === 'enter' && plain) ||
      (sendKey === 'auto' && plain && enterSends()) ||
      (sendKey === 'ctrl+enter' && ctrl && !shift) ||
      (sendKey === 'shift+enter' && shift && !ctrl);
    if (!explicit && !(ctrl && !shift)) {
      if (sendKey === 'auto' && plain) learnEnter(e);
      return;
    }
    const t = e.composedPath()[0];
  ...
  ST.guard = { closeDialog, enterSends, last: () => Object.assign({}, last) };
})();
