...
(function () {
  ...
  // id, сгенерированные фреймворком (React useId: base-ui-_r_fb_, :r1:, radix-…), меняются при перезагрузке
  function stableId(el) {
    return !!el.id && !/\d{3,}|_r_|^:r|:$|^(base-ui|radix|headlessui|mui|react-aria|ember)[-_]/i.test(el.id);
  }

  function uniqueSelector(el) {
    if (stableId(el)) return '#' + esc(el.id);
    const parts = [];
    let cur = el;
  ...
  ST.picker = {
    ...
    async run(kind) {
      ...
      if (kind === 'container') {
        ...
        const BTN = 'button,[role="button"],input[type="submit"],a';
        // Клик мимо самой кнопки (по обёртке) — берём единственную кнопку внутри
        let btn = ST.closestDeep(el, BTN);
        if (!btn) {
          const inner = el.querySelectorAll(BTN);
          btn = inner.length === 1 ? inner[0] : el;
        }
        // Короткий стабильный селектор по data-testid, если он однозначный
        const tid = btn.getAttribute('data-testid');
        const bySel = tid ? btn.tagName.toLowerCase() + '[data-testid="' + tid.replace(/"/g, '\\"') + '"]' : '';
        sel = bySel && ST.qsaSafe(bySel).length === 1 ? bySel : uniqueSelector(btn);
