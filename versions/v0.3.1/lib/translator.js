...
(function () {
  ...
  async function callProvider(provider, text, sl, tl, settings, opts) {
    let attempt = 0;
    for (;;) {
      checkCooldown(provider);
      await acquire();
      try {
        const r = await provider.translate({ text, sl, tl, settings, spell: !!(opts && opts.spell) });
  ...
  async function translateRaw(text, sl, tl, settings, opts) {
    ...
    const run = async (provider) => {
      ...
      const results = [];
      for (const c of chunks) results.push(await callProvider(provider, c, sl, tl, settings, opts));
      return {
   ...
   * opts.spell — запросить проверку орфографии исходного текста (только для текста оператора)
   */
  async function translate(text, sl, tl, settings, opts) {
    ...
    const p = translateRaw(protectedText.text, sl, tl, settings, opts)
      .then(async (r) => {
