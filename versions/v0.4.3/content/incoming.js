...
(function () {
  ...
  const SERVICE_HINT = /(actions|pill|header|meta|avatar|timestamp|footer|toolbar|reactions?)/i;

  function hintOf(el) {
    return (el.getAttribute('data-testid') || '') + ' ' + (typeof el.className === 'string' ? el.className : '');
  }
  ...
  function addMissedUserMessages(found) {
    ...
    for (const part of parts.slice(0, 200)) {
      ...
      for (const c of cands) {
        const len = ST.extractText(c).length;
        if (len < 2) continue;
        // предпочитаем самый внутренний, если он содержит почти весь текст внешнего
        if (!best || (best.el.contains(c) && len >= best.len * 0.6)) best = { el: c, len };
      }
      if (best) found.add(best.el);
    }
  }
