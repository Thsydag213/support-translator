...
(function () {
  ...
  function emptyDay(date) {
    return {
      ...
      // переводы встроенным переводчиком Chrome (без сети) и объединённые запросы
      local: { n: 0, chars: 0 }, batched: { requests: 0, items: 0 }
    };
  }
  ...
  const ST_STATS = {
    ...
    // n сообщений ушли одним запросом
    batch(n) {
      return mutate((d) => {
        d.batched = d.batched || { requests: 0, items: 0 };
        d.batched.requests += 1;
        d.batched.items += n;
      });
    },
    error(msg) {
    ...
    event(ev) {
      return mutate(async (d, a) => {
        if (ev.kind === 'incoming') {
          ...
          d.local = d.local || { n: 0, chars: 0 };
          d.local.n += 1;
          d.local.chars += ev.chars || 0;
          d.providers['chrome-builtin'] = (d.providers['chrome-builtin'] || 0) + (ev.chars || 0);
    ...
    // Символы, отправленные провайдеру в текущем календарном месяце (лимит Google Cloud считается помесячно)
    async monthChars(provider) {
      const month = L.today().slice(0, 7);
      const days = await this.getDays();
      return days.filter((d) => d.date.slice(0, 7) === month).reduce((n, d) => n + ((d.providers && d.providers[provider]) || 0), 0);
    },
    async today() {
      const days = await this.getDays();
      const t = L.today();
