...
(function () {
  ...
  // Валюта словом ↔ символом: «69,70 евро» и «69.70欧元» — то же, что «69,70 €».
  // Иначе сверка чисел ругалась бы на каждый перевод, где валюта записана иначе (китайский, японский, корейский, арабский…)
  const W = (s) => new RegExp('(?<![\\p{L}])(?:' + s + ')(?![\\p{L}])', 'giu');
  const CURRENCY_WORDS = [
    [W('евро|euros?|eur[oó]s?|€uro'), '€'], [/欧元|歐元|ユーロ|유로|يورو/g, '€'],
    [W('доллар\\p{L}*|dollars?|d[oó]lar(?:es)?|dollari|dólares'), '$'], [/美元|美金|ドル|달러|دولار/g, '$'],
    [W('фунт\\p{L}*(?:\\s+стерлинг\\p{L}*)?|pounds?(?:\\s+sterling)?|libras?(?:\\s+esterlinas?)?|sterlina|sterline'), '£'], [/英镑|英鎊|ポンド|파운드|جنيه/g, '£'],
    [W('рубл\\p{L}*|rubles?|rublos?|roubles?|rubli'), '₽'], [/卢布|盧布|ルーブル|루블|روبل/g, '₽'],
    [W('yen|иен\\p{L}*|иена'), '¥'], [/日元|日圓|円/g, '¥'],
    [W('гривн\\p{L}*|hryvnias?'), '₴'], [W('лир\\p{L}*|lira|liras|lire'), '₺']
  ];
  function currencyWordsToSymbols(text) {
    let t = text;
    for (const [re, sym] of CURRENCY_WORDS) t = t.replace(re, ' ' + sym + ' ');
    return t;
  }
  ...
  function extractFacts(text) {
    ...
    return {
      ...
      symbols: currencyWordsToSymbols(rest).match(CURRENCY_SYMBOLS) || []
    };
  }
