...
(function () {
  ...
  ST.local = {
    ...
    toBcp,
    // После скачивания пакетов — забыть закэшированное «нужно скачать»
    resetCache() {
      availability.clear();
      missing.clear();
    },
    // Скачивание пакетов по клику оператора (Chrome требует жест пользователя). Только если API есть в скрипте сайта;
    // иначе — страница настроек. pairs: ['ru>zh', …], onProgress(pair, text)
    canDownloadHere: pageHasTranslator,
    download(pairs, onProgress) {
      const jobs = pairs.map((p) => {
        ...
        return self.Translator.create({
          ...
          targetLanguage: tgt,
          monitor(m) {
            m.addEventListener('downloadprogress', (e) => onProgress && onProgress(p, Math.round((e.loaded || 0) * 100) + '%'));
          }
        }).then(
          () => { onProgress && onProgress(p, 'готово'); return true; },
          (e) => { onProgress && onProgress(p, 'ошибка: ' + (e && e.message)); return false; }
        );
      });
