(async function () {
  ...
  $('palette').addEventListener('click', async () => {
    await tabCommand({ type: 'command', command: 'command-palette' });
    window.close();
  });
  $('summary').addEventListener('click', async () => {
    await tabCommand({ type: 'command', command: 'ticket-summary' });
    window.close();
  });
  ...
  // Сочетания клавиш по умолчанию не назначены (Ctrl+Shift+Y занято сайтом) — показываем, если оператор задал свои
  // в chrome://extensions/shortcuts. Промис-версия getAll работает и в Firefox/Safari.
  Promise.resolve()
    .then(() => chrome.commands.getAll())
    .then((cmds) => {
      const find = (n) => (cmds || []).find((x) => x.name === n && x.shortcut);
      const parts = [];
      const t = find('translate-reply');
      const p = find('command-palette');
      if (t) parts.push(t.shortcut + ' — перевод');
      if (p) parts.push(p.shortcut + ' — команды');
      $('shortcut').textContent = parts.length ? parts.join(' · ') : 'Сочетания клавиш: chrome://extensions/shortcuts';
    })
    .catch(() => {});

  // Новая версия и расход Google Cloud
  bg({ type: 'updateInfo', force: false }).then((r) => {
    const u = r && r.update;
    if (!u) return;
    $('updVer').textContent = u.latest;
    $('updCur').textContent = u.current;
    $('updLink').addEventListener('click', (e) => { e.preventDefault(); chrome.tabs.create({ url: u.url }); });
    $('updateCard').hidden = false;
  });
  bg({ type: 'cloudUsage' }).then((r) => {
    const u = r && r.usage;
    if (!u || (!u.configured && !u.used)) return;
    const pct = Math.round(u.share * 100);
    const el = $('cloudUsage');
    el.hidden = false;
    el.className = 'small' + (pct >= 100 ? ' err' : pct >= 80 ? ' warn' : ' muted');
    el.textContent = 'Google Cloud в этом месяце: ' + u.used.toLocaleString('ru') + ' из ' + u.limit.toLocaleString('ru') + ' символов (' + pct + '%)' +
      (pct >= 100 ? ' — лимит исчерпан' : pct >= 80 ? ' — скоро лимит' : '');
  });
