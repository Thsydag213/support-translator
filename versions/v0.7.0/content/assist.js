...
(function () {
  ...
  async function rewrite(englishText, mode) {
    const m = MODES[mode] || MODES.fix;
    const system =
      'You edit replies written by customer support agents. ' + m.task + ' ' +
  ...
  // Ключевые слова: английский, испанский, португальский, французский, немецкий, итальянский, русский, украинский,
  ...
  function intent(text) {
    const t = String(text || '');
    const scores = INTENTS.map((x) => ({ x, n: (t.match(new RegExp(x.re.source, 'gi' + (x.re.flags.includes('m') ? 'm' : ''))) || []).length }))
      .filter((s) => s.n > 0)
      .sort((a, b) => b.n - a.n);
  ...
  ST.assist = { info, explain, rewrite, MODES, intent, currentIntent, macrosFor, analyzeTicket, openTicketPanel, closePanel };
})();
