  ...
  ST.diagnose = function () {
    ...
    if (site) {
      out.found = {
        ...
        localTranslator: {
          setting: ST.settings.localTranslatorMode,
          backend: ST.local ? ST.local.lastBackend() : '',
          bridge: ST.local ? ST.local.bridgeInfo() : null,
        ...
        // Помощник агента (без текста сообщений)
        helper: {
          quickInsert: !!ST.settings.quickInsert,
          translationMemory: ST.settings.tmEnabled !== false,
          assist: ST.settings.assist !== false,
          pageModelApi: typeof self.LanguageModel !== 'undefined',
          intent: (() => { try { const i = ST.assist && ST.assist.currentIntent(); return i ? i.id : ''; } catch (e) { return ''; } })(),
          langChangeNotice: ST.settings.langChangeNotice !== false,
          selectorCheck: ST.settings.selectorCheck !== false
        },
        expectedLang: ST.expectedLang(),
        // Последнее решение защиты от отправки (без текста сообщения)
        guard: ST.guard && ST.guard.last ? Object.assign(ST.guard.last(), { sendKey: (ST.site && ST.site.sendKey) || 'auto', sendGuard: !!(ST.site && ST.site.sendGuard !== false), agoSec: ST.guard.last().at ? Math.round((Date.now() - ST.guard.last().at) / 1000) : null }) : null,
