(async function () {
  ...
  // Сочетание клавиш по умолчанию не назначено (Ctrl+Shift+Y занято сайтом) — показываем, если оператор задал своё в chrome://extensions/shortcuts
  try {
    chrome.commands.getAll((cmds) => {
      const c = (cmds || []).find((x) => x.name === 'translate-reply');
      $('shortcut').textContent = c && c.shortcut ? c.shortcut + ' — перевод ввода' : 'Перевод ответа: кнопка «🌐 Перевести» у поля ввода';
    });
