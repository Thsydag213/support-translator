...
(function () {
  ...
  let lastInsert = null; // { field, original } — для "вернуть оригинал"
  let fab = null; // плавающая кнопка
  let panel = null; // открытая панель

  function fieldAllowed(el) {
    if (!ST.site || !ST.settings.enabled) return false;
  ...
  function ensureFab() {
    ...
    shadow.innerHTML = `
      ...
      <button type="button" title="Перевести введённый текст (Ctrl+Shift+Y)">🌐 Перевести</button>`;
  ...
  function placeFab() {
    if (!fab || !lastField || !document.contains(lastField)) return hideFab();
  ...
  const PANEL_CSS = `
    ...
    *{box-sizing:border-box}
    ...
    label{display:block;font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.04em;color:#5f6368;margin-bottom:4px}
  ...
  async function open(field) {
    ...
    const s = ST.settings;
    const target = ST.incoming.lastForeignLang() || ST.state.conversationLang ||(ST.site && ST.site.lastOutgoingLang) || s.fallbackOutgoingLang;
    ...
    shadow.innerHTML = `
      ...
      <div class="wrap" role="dialog" aria-label="Подтверждение перевода">
        ...
        <div class="grid">
          <div>
            <label for="orig">Ваш текст (оригинал)</label>
            <textarea id="orig"></textarea>
          </div>
          <div>
            <label for="tr">Перевод — будет вставлен в поле <span class="spin" id="spin" hidden>⏳</span></label>
            <textarea id="tr" class="tr" placeholder="Перевод…"></textarea>
          </div>
          <div class="warn" id="warn" hidden></div>
          <div class="err" id="err" hidden></div>
          <div style="grid-column:1/-1">
            <label>Обратный перевод — проверка смысла <span class="lang" id="backLang"></span> <span class="spin" id="spinBack" hidden>⏳</span></label>
    ...
    const ui = {
      ...
      warn: $('warn'), err: $('err'), ok: $('ok'), spin: $('spin'), spinBack: $('spinBack')
    };
    ...
    const state = { srcLang: '', seq: 0, backSeq: 0, trEditedByUser: false };
    panel = { host, field };
    ...
    ui.tr.addEventListener('input', () => {
      state.trEditedByUser = true;
      ui.ok.disabled = !ui.tr.value.trim();
      clearTimeout(tTr);
      tTr = setTimeout(doBack, 700);
    });
    ui.tl.addEventListener('change', doTranslate);
    ...
    async function confirm() {
      ...
      lastInsert = { field, original: originalNow };
      rememberLang(tl);
  ...
  function restoreOriginal() {
    if (!lastInsert || !document.contains(lastInsert.field)) return;
  ...
  ST.outgoing = {
    openForActive() {
      const a = document.activeElement;
      const f = a && fieldAllowed(a) ? a : lastField;
