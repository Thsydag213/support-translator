(async function () {
  ...
  function mergeDays(lists) {
    ...
    for (const list of lists) {
      for (const d of list) {
        ...
        m.local.n += (d.local && d.local.n) || 0;
        m.local.chars += (d.local && d.local.chars) || 0;
        m.batched.requests += (d.batched && d.batched.requests) || 0;
        m.batched.items += (d.batched && d.batched.items) || 0;
      }
    }
  ...
  function renderTiles(days) {
    ...
    const tiles = [
      ...
      [fmt(days.reduce((n, d) => n + ((d.local && d.local.n) || 0), 0)), 'переведено на устройстве', fmt(days.reduce((n, d) => n + ((d.local && d.local.chars) || 0), 0)) + ' символов без сети'],
      [fmt(days.reduce((n, d) => n + ((d.batched && d.batched.items) || 0), 0)), 'сообщений объединено', 'в ' + fmt(days.reduce((n, d) => n + ((d.batched && d.batched.requests) || 0), 0)) + ' запросов']
    ];
