(async function () {
  ...
  const GLOBAL_SELECTS = ['formality', 'uiTheme', 'localTranslator'];
  ...
  function localPairs() {
    collect();
    const tgt = toBcp(settings.targetLang);
    return L.splitList($('localLangs').value).map(toBcp).filter((s) => s && s !== tgt).map((s) => [s, tgt]);
  }
  ...
  $('localDownload').addEventListener('click', () => {
    ...
    const progress = {};
    const render = () => {
      $('localOut').textContent = pairs.map(([s, t]) => L.langName(s) + ' → ' + L.langName(t) + ': ' + (progress[s] || '⏳')).join('\n');
    };
      ...
      Translator.create({
        ...
        monitor(m) {
          m.addEventListener('downloadprogress', (e) => {
            progress[s] = '⏳ ' + Math.round((e.loaded || 0) * 100) + '%';
            render();
          });
        }
      }).then(() => { progress[s] = '✅ готово'; }, (e) => { progress[s] = '— ' + (e && e.message ? e.message : 'недоступно'); })
    );
