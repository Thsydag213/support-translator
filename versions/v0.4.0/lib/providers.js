...
(function () {
  ...
  const googleFree = {
    ...
    // Несколько текстов одним запросом: склеиваем через метки-разделители ⟦900001⟧ (метки вида ⟦n⟧ Google не трогает).
    // Если после перевода частей не столько же, сколько текстов — ошибка 422, сервис переведёт по одному.
    async translateMany({ texts, sl, tl, settings }) {
      if (texts.length === 1) return [await this.translate({ text: texts[0], sl, tl, settings })];
      const joined = texts.map((t, i) => (i ? '\n\n⟦' + (900000 + i) + '⟧\n\n' : '') + t).join('');
      const r = await this.translate({ text: joined, sl, tl, settings });
      const parts = r.translation.split(/\s*⟦\s*900\d{3}\s*⟧\s*/);
      if (parts.length !== texts.length) throw new ProviderError('Не удалось разделить объединённый перевод', 422);
      return parts.map((p) => ({ translation: p.trim(), detectedLang: r.detectedLang }));
    }
  };
  ...
  const googleCloud = {
    ...
    // API принимает массив q — каждый текст переводится и определяется отдельно
    async translateMany({ texts, sl, tl, settings }) {
      const key = settings && settings.googleCloudApiKey;
      if (!key) throw new ProviderError('Не задан API key Google Cloud', 401);
      const body = { q: texts, target: tl, format: 'text' };
      if (sl && sl !== 'auto') body.source = sl;
      let res;
      try {
        res = await fetch('https://translation.googleapis.com/language/translate/v2?key=' + encodeURIComponent(key), {
      ...
      if (!res.ok) throw new ProviderError('Google Cloud HTTP ' + res.status, res.status);
      const data = await res.json();
      return data.data.translations.map((t) => ({ translation: decodeEntities(t.translatedText), detectedLang: t.detectedSourceLanguage || sl || '' }));
    }
  };
  ...
  const devMock = {
    ...
    async translateMany({ texts, sl, tl }) {
      devMock.manyCalls = (devMock.manyCalls || 0) + 1; // счётчик для тестов объединения
      return Promise.all(texts.map((text) => devMock.translate({ text, sl, tl })));
    }
  };
