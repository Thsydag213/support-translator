...
(function () {
  ...
  const DEFAULTS = {
    ...
    spellCheck: true,
    // Встроенный переводчик Chrome (на устройстве, без сети и без лимитов):
    //   'incoming+back' — входящие и обратный перевод в панели | 'incoming' — только входящие | 'off' — не использовать.
    // Если языковой пакет не скачан или Chrome не поддерживает API — автоматически используется облачный провайдер
    localTranslator: 'incoming+back',
