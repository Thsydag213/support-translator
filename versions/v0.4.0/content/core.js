...
(function () {
  ...
  ST.translate = async function (text, sl, tl, opts) {
    opts = opts || {};
    if (opts.local && ST.local && ST.settings.localTranslator !== 'off') {
      const src = sl && sl !== 'auto' ? sl : opts.localSource;
      const r = await ST.local.translate(text, src, tl).catch(() => null);
      if (r) {
        if (!r.fromCache) ST.track({ kind: 'local', chars: String(text).length });
        return r;
      }
    }
    const cloudOpts = { spell: !!opts.spell, batch: !!opts.batch, hint: opts.hint || '' };
    const resp = await ST.send({ type: 'translate', text, sl: sl || 'auto', tl, opts: cloudOpts });
    if (!resp || !resp.ok) {
