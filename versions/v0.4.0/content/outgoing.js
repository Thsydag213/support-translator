...
(function () {
  ...
  const PANEL_CSS = `
    ...
    *{box-sizing:border-box}
    ...
    textarea{width:100%;min-height:110px;resize:vertical;font:inherit;padding:8px 10px;border:1px solid var(--border);border-radius:8px;color:inherit;background:var(--bg)}
    textarea:focus{outline:2px solid var(--accent);border-color:transparent}
    textarea.tr{background:var(--trbg);border-color:var(--trborder)}
    .back{grid-column:1/-1;background:var(--soft);border:1px dashed var(--border);border-radius:8px;padding:8px 10px;white-space:pre-wrap;min-height:40px}
  ...
  async function open(field, opts) {
    ...
    const state = { srcLang: '', seq: 0, backSeq: 0, spell: '', sameLang: false, politeTried: false };
    panel = { host, field };
    ...
    async function doBack() {
      ...
      try {
        ...
        const b = await ST.translate(text, ui.tl.value, backLang, { local: s.localTranslator === 'incoming+back' });
    ...
    async function doTranslate() {
      ...
      try {
        const res = await ST.translate(text, 'auto', tl, { spell: !!s.spellCheck });
