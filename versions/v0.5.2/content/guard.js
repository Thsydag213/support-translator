...
(function () {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    // Проверяем только сочетание, которым сайт отправляет; остальные Enter (новая строка) не трогаем
    const sends =
      (sendKey === 'enter' && !ctrl && !shift) ||
      (sendKey === 'ctrl+enter' && ctrl && !shift) ||
      (sendKey === 'shift+enter' && shift && !ctrl);
    if (!sends) return;
    const t = e.composedPath()[0];
    if (!ST.fieldAllowed(t)) return;
    intercept(e, { kind: 'key', field: ST.editableRoot(t), ctrl: sendKey === 'ctrl+enter', shift: sendKey === 'shift+enter' });
  }, true);
