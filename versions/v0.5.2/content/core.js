...
(function () {
  ...
  ST.getFieldText = function (el) {
    ...
    const isBlock = (k) => FIELD_BLOCK.test(k.tagName) || /^(UL|OL)$/.test(k.tagName);
    if (kids.length && kids.every(isBlock)) {
      const lines = [];
      for (const k of kids) {
        if (/^(UL|OL)$/.test(k.tagName)) {
          ...
          Array.from(k.children).filter((li) => li.tagName === 'LI').forEach((li, i) => {
            ...
            lines.push(marker + (li.innerText || '').replace(/\n+/g, ' ').trim());
          });
        } else {
          lines.push((k.innerText || '').replace(/\n$/, ''));
        }
      }
      return lines.join('\n');
    }
