...
(function () {
  ...
  ST.translate = async function (text, sl, tl) {
    ...
    if (!resp || !resp.ok) {
      const msg = (resp && resp.error) || 'Нет ответа от расширения';
  ...
  function isActiveIndicator(el) {
    for (let cur = el, i = 0; cur && i < 3; cur = cur.parentElement, i++) {
      if (cur.getAttribute('aria-selected') === 'true' || cur.getAttribute('aria-pressed') === 'true') return true;
      if (cur.hasAttribute('aria-current') || /^(active|on|checked)$/.test(cur.getAttribute('data-state') || '')) return true;
      if (cur.hasAttribute('aria-haspopup') || cur.hasAttribute('aria-expanded')) return true; // кнопка-выпадашка показывает текущий режим
      if (/(^|[\s_-])(active|selected|current|is-active|checked)($|[\s_-])/i.test(cur.className && cur.className.baseVal == null ? cur.className : '')) return true;
    }
    return false;
  }
  ...
  ST.isNoteMode = function (field) {
    ...
    if (site.noteSelector) {
      try {
        if (ST.closestDeep(field, site.noteSelector)) return true;
        if (ST.qsa(site.noteSelector).some((e) => e.getClientRects().length > 0)) return true;
      } catch (e) { /* неверный селектор */ }
    }
    ...
    for (let level = 0; level < 8; level++) {
      ...
      for (const el of all) {
        ...
        if (el.getClientRects().length && isActiveIndicator(el)) return true;
      }
    }
    return false;
  };
  ...
  ST.getFieldText = function (el) {
    ...
    if (kids.length && kids.every((k) => FIELD_BLOCK.test(k.tagName))) {
      return kids.map((k) => (k.innerText || '').replace(/\n$/, '')).join('\n');
    }
