 ...
 *   translate({ text, sl, tl, settings }) -> Promise<{ translation, detectedLang, spell? }>
...
(function () {
  ...
  const googleFree = {
    ...
    async translate({ text, sl, tl }) {
      const url =
        // dt=t — перевод, dt=qca — проверка орфографии исходного текста
        'https://translate.googleapis.com/translate_a/single?client=gtx&dt=t&dt=qca&dj=1' +
      ...
      const spell = data.spell && typeof data.spell.spell_res === 'string' ? data.spell.spell_res : '';
      return { translation, detectedLang: data.src || '', spell };
    }
  };
