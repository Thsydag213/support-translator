...
(function () {
  const SITE_DEFAULTS = {
    ...
    sendButtonSelector: '',
    // Как сайт отправляет сообщение с клавиатуры: 'enter' | 'ctrl+enter' | 'shift+enter' | 'none'
    sendKey: 'enter',
  ...
  const REPLY_LANGS_DEFAULT = ['en', 'fr', 'pt', 'it', 'es', 'de', 'ko', 'ja', 'ar'];
  ...
  function replyAllowed(settings, code) {
    ...
    return list.some((x) => sameLang(x, code));
  }
  ...
  globalThis.ST_LIB = {
    ...
    urlMatches, splitList, replyAllowed, replyTarget, REPLY_LANGS_DEFAULT, ruleOrigins, isValidMatchPattern, findSiteRule, mergeSettings, hash, today
  };
})();
