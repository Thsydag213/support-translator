...
(function () {
  ...
  function containers() {
    ...
    if (!sel) return document.body ? [document.body] : [];
    try {
  ...
  function messages() {
    ...
    for (const c of containers()) {
      if (EMBEDDED && c === document.body) {
        const inner = msgSel ? safeQuery(c, msgSel) : [];
  ...
  function cleanupOrphans() {
    document.querySelectorAll('[' + ST.UI_ATTR + '="tr"],[' + ST.UI_ATTR + '="err"]').forEach((node) => {
  ...
  function renderError(el, err) {
    ...
    const node = ST.h('div', { [ST.UI_ATTR]: 'err', class: 'st-tr st-tr--error' },
      ...
      ST.h('button', { class: 'st-tr__btn', type: 'button', onclick: (e) => { e.stopPropagation(); el.removeAttribute(DONE); removeOurNode(el); processMessage(el); } }, 'повторить')
    );
    place(el, node);
  }
  ...
  ST.incoming = {
    ...
    reset() {
      ...
      document.querySelectorAll('[' + ST.UI_ATTR + '="tr"],[' + ST.UI_ATTR + '="err"]').forEach((n) => n.remove());
    },
    rescan() {
      this.reset();
      scan();
    ...
    lastForeignLang() {
      ...
      for (let i = list.length - 1; i >= 0; i--) {
        ...
        if (userSel) {
          let isUser = false;
          try { isUser = el.matches(userSel) || !!el.closest(userSel); } catch (e) { isUser = true; }
          if (!isUser) continue;
