...
const CS_CSS = ['content/content.css'];
...
async function injectTab(tabId) {
  const run = async (allFrames) => {
    await chrome.scripting.insertCSS({ target: { tabId, allFrames }, files: CS_CSS });
...
chrome.tabs.onRemoved.addListener((tabId) => chrome.storage.session.remove('frames:' + tabId));
