...
(function () {
  ...
  function findField() {
    ...
    if (!sel) return null;
    try {
      const all = Array.from(document.querySelectorAll(sel)).filter((el) => el.offsetParent !== null);
  ...
  function resend(ctx) {
    ...
    if (notCancelled && ST.site.sendButtonSelector) {
      try {
        const btn = document.querySelector(ST.site.sendButtonSelector);
  ...
  async function detect(text, expected) {
    ...
    if (local.reliable) return local.lang;
  ...
  function showWarning(ctx, text, exp, detected) {
    ...
    shadow.innerHTML = `
      <style>
        :host{all:initial}
        .box{position:fixed;z-index:2147483647;width:min(440px,calc(100vw - 24px));background:#fff;color:#202124;border-radius:12px;
          border:1px solid #f0c36d;box-shadow:0 12px 40px rgba(0,0,0,.28);font:13px/1.45 system-ui,-apple-system,Segoe UI,sans-serif;padding:14px}
        b{display:block;font-size:14px;margin-bottom:6px;color:#7a4f01}
        p{margin:0 0 12px;color:#3c4043}
        .row{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap}
        button{font:600 13px/1 system-ui,sans-serif;padding:8px 12px;border-radius:8px;border:1px solid #dadce0;background:#fff;color:#202124;cursor:pointer}
  ...
  window.addEventListener('click', (e) => {
    ...
    let button = null;
    try {
      button = t && t.closest ? t.closest(ST.site.sendButtonSelector) : null;
    } catch (err) {
      return;
    }
    if (!button) return;
